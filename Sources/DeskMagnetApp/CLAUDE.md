# DeskMagnetApp/
> L2 | 父级: /Users/luo/Desktop/ClaudeCode/web/DeskMagnet/Sources/CLAUDE.md

成员清单
AppDelegate.swift: AppKit 生命周期，创建固定 800x520 亮色主窗口、隐藏原生标题栏文字、转换 Finder 顶左坐标、关闭时自动恢复、启动未恢复状态检测
ContentView.swift: SwiftUI 内容区，以统一 canvas padding 呈现亮色“桌面清理大师”工具面板，不暴露吸附实现
DeskMagnetAppMain.swift: NSApplication 入口，设置 delegate 并启动事件循环
DeskMagnetViewModel.swift: 主窗口状态模型，调用 AppCoordinator 执行吸附、关闭恢复、跟随同步并呈现错误
WindowFollowController.swift: 监听 NSWindow.didMoveNotification，将 AppKit 窗口坐标转为 Finder 桌面坐标，按图标数量动态节流并 final sync

模块法则:
App 层只处理窗口、弹窗与文案；Finder 副作用必须通过 DeskMagnetCore 的 AppCoordinator。

[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
